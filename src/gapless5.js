...
var gapless5AudioContext = window.hasWebKit
...
function Gapless5Source(parentPlayer, inContext, inOutputNode) {
  ...
  var playAudioFile = function(force) {
    ...
    if (buffer != null) {
      ...
      if (window.hasWebKit) source.start(0, offsetSec, restSec);
      else source.start(0, offsetSec);
  ...
  this.isPlayActive = function() {
    return (
      (that.inPlayState() || queuedState == Gapless5State.Play) &&
      !that.audioFinished
    );
  };
...
var Gapless5FileList = function(inPlayList, inStartingTrack, inShuffle) {
  ...
  var addIndices = function(inputList) {
    ...
    for (var n = 0; n < temp.length; n++) temp[n]._index = n + 1;
    return temp;
  };
  ...
  var shuffle = function(inputList, index) {
    ...
    if (swapIndex != 0) swapElements(outputList, swapIndex, 0);
  ...
  var addFile = function(point, file, list, listShuffled) {
    ...
    for (var i = 0; i < list.length; i++)
      if (list[i]._index >= addin._index) list[i]._index = list[i]._index + 1;

    // If shuffle mode, new index should be array size so
    // unshuffled mode puts it at the back of the array.
    if (listShuffled == true) list.push(addin);
    else list.splice(point, 0, addin);
  };
  ...
  var removeFile = function(point, list, listShuffled) {
    if (listShuffled == true) {
      for (var j = 0; j < list.length; j++)
        if (list[j]._index == point + 1) list.splice(j, 1);
    } else list.splice(point, 1);

    // After removing the item, re-number the indexes
    for (var k = 0; k < list.length; k++)
      if (list[k]._index >= point + 1) list[k]._index = list[k]._index - 1;
  };
  ...
  this.lastIndex = function(index, newList, oldList) {
    ...
    for (var n = 0; n < oldList.length; n++)
      ...
      if (oldList[n]._index == compare._index) return n;
  ...
  this.shuffleToggle = function() {
    if (remakeList == true) return revertShuffle();

    if (shuffleMode == false) return enableShuffle();

    if (shuffleMode == true) return disableShuffle();
  };
  ...
  this.rebasePlayList = function(index) {
    if (shuffleMode == true) that.current = reorder(that.current, index);
  ...
  this.getIndex = function(index) {
    if (that.shuffled()) {
      for (var i = 0; i < that.current.length; i++)
        if (that.current[i]._index == index) return i - 1;
    } else return index;
  };
  ...
  this.add = function(index, file) {
    ...
    if (remakeList == true) addFile(index, file, that.previous, !shuffleMode);
    else addFile(index, file, that.previous, shuffleMode);

    // Shift currentItem if the insert file is earlier in the list
    if (index <= that.currentItem) that.currentItem = that.currentItem + 1;
  ...
  this.remove = function(index) {
    ...
    if (remakeList == true) removeFile(index, that.previous, !shuffleMode);
    else removeFile(index, that.previous, shuffleMode);
    ...
    if (index < that.currentItem || index >= that.previous.length - 1)
      if (that.currentItem > 0) that.currentItem = that.currentItem - 1;
  ...
  this.files = function() {
    return that.current.map(function(song) {
      return song.file;
    });
  };
...
var Gapless5 = function(elem_id, options) {
  ...
  this.useWebAudio =
    options != null && "useWebAudio" in options ? options.useWebAudio : true;
  this.useHTML5Audio =
    options != null && "useHTML5Audio" in options
      ? options.useHTML5Audio
      : !isMobileBrowser;
  ...
  var gainNode =
    window.hasWebKit || typeof AudioContext != "undefined"
      ? context.createGain()
      : null;
  if (context && gainNode) {
  ...
  var getUIPos = function() {
    var position = isScrubbing
      ? scrubPosition
      : that.mgr.sources[dispIndex()].getPosition();
    return (
      (position / that.mgr.sources[dispIndex()].getLength()) * SCRUB_RESOLUTION
    );
  };

  var getSoundPos = function(uiPosition) {
    return (
      (uiPosition / SCRUB_RESOLUTION) *
      that.mgr.sources[dispIndex()].getLength()
    );
  };

  var numTracks = function() {
    // FileList object must be initiated
    if (that.trk != null) return that.trk.current.length;
    else return 0;
  };
  ...
  var index = function() {
    // FileList object must be initiated
    if (that.trk != null) return that.trk.get();
    else return -1;
  };
  ...
  var dispIndex = function() {
    if (readyToRemake()) return that.trk.previousItem;
    else if (that.trk != null) return that.trk.get();
    else return -1;
  };

  var readyToRemake = function() {
    // FileList object must be initiated
    if (that.trk.readyToRemake() != null) return that.trk.readyToRemake();
    else return false;
  };
  ...
  var getTotalPositionText = function() {
    ...
    if (numTracks() == 0) {
      ...
      that.mgr.sources[dispIndex()].getState() == Gapless5State.Error
    ) {
      text = ERROR_TEXT;
    } else if (srcLength > 0) {
  ...
  this.onFinishedScrubbing = function() {
    ...
    if (
      that.mgr.sources[dispIndex()].inPlayState() &&
      newPosition >= that.mgr.sources[dispIndex()].getLength()
    ) {
      that.next(true);
    } else {
  ...
  this.insertTrack = function(point, audioPath) {
    ...
    if (point == trackCount) {
      ...
      that.mgr.sources.splice(
        point,
        0,
        new Gapless5Source(this, context, gainNode)
      );
      // TODO: refactor to take an entire JSON object
      // TODO: move this function to the fileList object
  ...
  var updateDisplay = function() {
    if (numTracks() == 0) {
      ...
      enableButton(
        "prev",
        that.loop || index() > 0 || that.mgr.sources[index()].getPosition() > 0
      );
      ...
      if (that.trk.current.length > 2) isShuffleActive = true;

      if (that.trk.shuffled()) shuffleButton("unshuffle", isShuffleActive);
      else shuffleButton("shuffle", isShuffleActive);
  ...
  var Init = function(elem_id, options, tickMS) {
    if ($("#" + elem_id).length == 0) {
      console.log(
        "ERROR in Gapless5: no element with id '" + elem_id + "' exists!"
      );
      return;
    }
    ...
    player_html +=
      ...
      that.id +
      '">00:00.00</span> | <span id="totalPosition' +
      that.id +
      '">' +
      LOAD_TEXT +
      "</span>";
    ...
    player_html +=
      '<div class="g5meter"><span id="loaded-span' +
      that.id +
      '" style="width: 0%"></span></div>';

    player_html +=
      '<input type="range" class="transportbar" name="transportbar" id="transportbar' +
      that.id +
      '" ';
    player_html +=
      'min="0" max="' +
    ...
    player_html +=
      'onmousedown="' +
      PlayerHandle() +
      '.onStartedScrubbing();" ontouchstart="' +
      PlayerHandle() +
      '.onStartedScrubbing();" ';
    player_html +=
      'onmouseup="' +
      PlayerHandle() +
      '.onFinishedScrubbing();" ontouchend="' +
      PlayerHandle() +
      '.onFinishedScrubbing();" />';
    ...
    player_html +=
      '<button class="g5button g5shuffle" id="shuffle' + that.id + '"/>';
    ...
    if (isMobileBrowser) {
      ...
      player_html +=
        '<input type="range" class="volume" name="gain" min="0" max="' +
        SCRUB_RESOLUTION +
        '" value="' +
        SCRUB_RESOLUTION +
        '" oninput="' +
    ...
    $("#prev" + that.id)[0].addEventListener(
      "mousedown",
      GAPLESS5_PLAYERS[that.id].prev
    );
    $("#play" + that.id)[0].addEventListener(
      "mousedown",
      GAPLESS5_PLAYERS[that.id].playpause
    );
    $("#stop" + that.id)[0].addEventListener(
      "mousedown",
      GAPLESS5_PLAYERS[that.id].stop
    );
    $("#shuffle" + that.id)[0].addEventListener(
      "mousedown",
      GAPLESS5_PLAYERS[that.id].shuffleToggle
    );
    $("#next" + that.id)[0].addEventListener(
      "mousedown",
      GAPLESS5_PLAYERS[that.id].next
    );
    ...
    if (
      options != null &&
      "shuffleButton" in options &&
      options.shuffleButton != true
    ) {
    ...
    var shuffleInit =
      options != null && "shuffle" in options && options.shuffle == true;
